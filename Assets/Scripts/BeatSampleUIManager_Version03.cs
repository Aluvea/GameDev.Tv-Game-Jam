...
public class BeatSampleUIManager_Version03 : BeatSampleUIManager
{

    [SerializeField] BeatSampleUI_version2 beatSamplePrefab;

    [Range(-360.0f,360.0f)]
    [SerializeField] float startingZRotation = 0.0f;
    [Range(-360.0f, 360.0f)]
    [SerializeField] float endingZRotation = 0.0f;
    ...
    private static BeatSampleUIManager_Version03 CurrenBeatSampleUIManager
    {
        set;
        get;
    }
    ...
    /// </summary>
    Queue<BeatSampleUI_version2> beatSampleObjectPool = new Queue<BeatSampleUI_version2>();

    /// <summary>
    /// List of currently playing beat sample UIs
    /// </summary>
    List<BeatSampleUI_version2> playingBeatSamplePool = new List<BeatSampleUI_version2>();
    ...
    public static void OnBeatSampleUIAnimationEnded(BeatSampleUI_version2 beatSampleUI)
    {
        CurrenBeatSampleUIManager.ResetBeatInstance(beatSampleUI);
    }

    private void ResetBeatInstance(BeatSampleUI_version2 beatSample)
    {
        ...
        beatSample.transform.eulerAngles = Vector3.zero;
        // Restart its color
    ...
    public override void DisplayBeat(BeatSyncData beatSyncData)
    {
        ...
        {
            BeatSampleUI_version2 dequeuedBeatSample = beatSampleObjectPool.Dequeue();
            dequeuedBeatSample.AnimateBeatData(beatSyncData, startingZRotation, endingZRotation, endingDirection);
            playingBeatSamplePool.Add(dequeuedBeatSample);
            return;
        }
        
        // Otherwise, instantiate a beat sample UI
        BeatSampleUI_version2 beatSample = Instantiate(beatSamplePrefab, startingBeatUIPosition.position, Quaternion.identity, beatUIParent);
        // Set its starting color
        beatSample.SetBeatSampleColor(startingInnerCircleColor, startingOuterCircleColor);
        // Animate the beat beat sync data towards the ending beat ui position
        beatSample.AnimateBeatData(beatSyncData, startingZRotation, endingZRotation, endingDirection);
    ...
    public override void UpdateBeatSyncUIState(BeatSyncData beatHit, BeatInputSync beatInput)
    {
        ...
        foreach (BeatSampleUI_version2 playingSample in playingBeatSamplePool)
        {
